...
def process_signal_for_date(target_date_str):
    ...
    if df_daily_for_date.empty:
        ...
        df_hourly.sort_values("datetime", ascending=True, inplace=True)
        df_30days = df_hourly[
            (df_hourly["datetime"] >= start_14_days)
...
It should be a clear price peak within the provided hourly data. Prefer a high tested multiple times or that caused a substantial pullback.

Support Level: Identify the most recent prominent swing low, a clear price level where downward movement stopped, and price moved significantly upward afterward. Prefer a low tested multiple times or that caused a substantial bounce.
...
start_date = end_date - pd.DateOffset(days=1)  # 8 months prior
...
for current_date in date_range:
    ...
    if "NoSignal" in result:
        ...
        row_data = {"Date": result["Date"], "NoSignal": result["NoSignal"]}
        df_trade = pd.DataFrame([row_data])
        df_trade.to_csv(csv_filename, mode="a", index=False, header=write_header)
        write_header = False
...
print(
    f"Done! Check {csv_filename} for all signals from {start_date.date()} to {end_date}."
)
